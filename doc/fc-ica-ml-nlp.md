...
```
AI의 세계
├── 고전적 머신러닝 (Classical ML)  ← 오늘의 주인공
│   ├── 회귀 (Regression)
│   ├── 분류 (Classification)
│   └── 군집화 (Clustering)
├── 딥러닝 (Deep Learning)
│   ├── CNN, RNN, Transformer...
│   └── LLM (GPT, Claude 등)  ← 여러분이 이미 아는 것
└── 고전적 자연어 처리 (Classical NLP)  ← 오늘의 주인공
    ├── 형태소 분석
    ├── 키워드/연관어 분석
    ├── 키워드 네트워크 분석
    ├── 토픽 분석 (LDA)
...
```
1단계: "그룹을 3개로 나눠봐" (K=3 지정)
2단계: 임의로 3개 중심점 배치
3단계: 각 데이터를 가장 가까운 중심점 그룹에 배정
4단계: 그룹 내 평균으로 중심점 이동
5단계: 변화 없을 때까지 3~4 반복
→ 결과: 자연스럽게 3개의 고객 군집 완성!
```
...
```
문서 수집 → 형태소 분석 → TF-IDF 벡터 변환
    → K-Means 군집화 → 각 군집의 대표 키워드 추출 → 이름 붙이기(사람이)
```
...
[0, 0, 0, 128, 255, 200, 0, ...]
    ↓
분류 모델 (Random Forest / SVM / CNN)
    ↓
출력: "이 이미지는 '7'일 확률 98.3%"
```
...
```
앱 시작
  ├── model/mnist_rf.pkl 존재 → 즉시 로드
  └── 없음 → MNIST 학습 후 저장 (최초 1회)

브라우저에서
  캔버스에 숫자 그리기 (검정 배경 + 흰 선, MNIST 형식과 동일)
      ↓
  [예측] 버튼 → base64 PNG를 /predict 엔드포인트로 전송
      ↓
  서버: 28×28 리사이즈 → 정규화 → 모델 추론
      ↓
  예측 숫자 + 0~9 확률 막대 표시 (최고 확률 항목 강조)
```
